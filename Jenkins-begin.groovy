...
node ('build-zenoss-product') {
    ...
    try {
        ...
        sh("./build_status.py -b ${BRANCH} -p ${PRODUCT_BUILD_NUMBER} --job-name ${env.JOB_BASE_NAME} --job-status ${currentBuild.result} --job-url ${env.BUILD_URL} -html buildReport.html")
