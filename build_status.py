...
class JobTemplate(object):
    ...
    Attributes:
        name (str) - The name of this job template (not necessarily the same
            as the job name).
        jobPrefixes ((:obj:`list` of :obj:`str`)) - A list of of job name
            prefixes used in cases where a single instance of this report
            template should be applied to multiple types of jobs that differ
            only by name.  If empty, then 'name' will be used to match this
            template to a particular type of job in Jenkins.
            Should not be used in conjunction with instanceTemplates.
        stages (:obj:`list` of :obj:`StageTemplate`) - list of the stages
            that will be reported for these types of job.
        instanceTemplates (:obj:`list` of :obj:`InstanceTemplate`) - list of
            instance report templates. Only used to customize the report when
            different instances of the same type of job have different
            reporting requirements.  Should not be used in conjunction with
            jobPrefixes.
    """
    ...
    def __str__(self):
        return "JobTemplate name '%s', jobPrefixes %s" % (
            self.name, self.jobPrefixes
        )
...
class InstanceTemplate(object):
    ...
    Attributes:
        jobPrefix (str) - The prefix for job names to use for this report
            template.
        ignoreStages (:obj:`list` of :obj:`str`) - A list of stages to be
            ignored for jobs which match this template.
...
class JobReport(object):
    ...
    StageInfo objects, which may contain a list of one or more JobReport
    objects in cases where the stage spawns child jobs.

    Attributes:
        jenkinsInfo (:obj:`list` of :obj:`JenkinsInfo`) - Information about the
            Jenkins job itself (name, label, number, etc)
        timeStats (:obj:`list` of :obj:`TimeStats`) - Statistics on when the
            job started and how long it took to complete
        stages (:obj:`list` of :obj:`StageInfo`) - The list of stages for
            the job.
    """
    ...
    def __str__(self):
        return "JobReport name '%s', label '%s'" % (
            self.jenkinsInfo.name, self.jenkinsInfo.label
        )

    def toDict(self):
        return {
            "jenkinsInfo":
                self.jenkinsInfo.toDict() if self.jenkinsInfo else {},
...
class BuildInfo(object):
    ...
    Attributes:
        server (str) - HTTP server name (e.g. http://server)
        jobName (str) - Full path of job name (e.g. Folder/Job)
        baseName (str) - Base job name (e.g. Job)
        buildNumber (int) - The job instance (build number)
        jobPath (str) - URI path to Job instances
        buildUrl (str) - Full URL path to job instance (build)
    """

    def __init__(self, server, jobName, buildNumber):
        """
        """
        self.server = server
        self.baseName = jobName.split('/')[-1]
        self.jobName = jobName
        self.buildNumber = buildNumber

        parts = jobName.split('/')
        self.parentPath = os.path.join(*(
            itertools.chain.from_iterable(
                zip(["job"]*len(parts[:-1]), parts[:-1])
            )
        ))
        self.jobPath = os.path.join(*(
            itertools.chain.from_iterable(zip(["job"]*len(parts), parts))
        ))
        self.buildUrl = os.path.join(
            self.server, self.jobPath, str(self.buildNumber)
        )

    def __str__(self):
        return "BuildInfo Job '%s', build '%s'" % (
            self.jobName, self.buildNumber
        )
...
class JenkinsInfo(object):
    ...
    def __str__(self):
        return "JobReport name '%s', label '%s', status '%s'" % (
            self.name, self.label, self.status
        )
...
class StageInfo(object):
    ...
    Attributes:
        ...
        timeStats (:obj:`list` of :obj:`TimeStats`) - Statistics on when the
            stage started and how long it took to complete
...
def loadReportTemplates(templateFile, build):
    templates = json.loads(
        templateFile.read(), object_hook=class_mapper
    )['templates']
    ...
    for jobTemplate in templates:
        ...
        for stage in jobTemplate.stages:
            stage.childInfoUrl = \
                stage.childInfoUrl.replace("%parentPath%", build.parentPath)
            log.debug("\t%s", stage)
...
def getJobStages(templates, jobName, jobLabel):
    found = False
    ...
    log.debug(
        'found stages for job "%s" - "%s" in template %s',
        jobName, jobLabel, template.name
    )
...
def buildReport(templates, jobInfo, jobName, server):
    ...
    jenkinsInfo.url = urlparse.urljoin(server, baseUrl)
    jenkinsInfo.number = jobInfo["id"]
    jenkinsInfo.status = jobInfo["status"]
    log.debug("JenkinsInfo = %s", jenkinsInfo)

    stages = []
    jobStages = getJobStages(templates, jenkinsInfo.name, jenkinsInfo.label)
    for stageTemplate in jobStages:
        log.debug(
            "trying to find match for stageTemplate %s", stageTemplate.name
        )
        found = False
        for stage in jobInfo["stages"]:
            if stage["name"] == stageTemplate.name:
                ...
                if stageTemplate.childTemplate:
                    addChildJobs(stageTemplate, templates, stage, jobs, server)
                stageTime = TimeStats(
                    stage["startTimeMillis"], stage["durationMillis"]
                )
                stageInfo = StageInfo(
                    stage["name"], stage["status"], stageTime, jobs
                )
                break
        ...
        if not found:
            jobs = []
            if stageTemplate.childTemplate:
                jobs = addChildJobTemplates(
                    templates, stageTemplate.childTemplate, jenkinsInfo.name
                )
    ...
    timeStats = TimeStats(
        jobInfo["startTimeMillis"], jobInfo["durationMillis"]
    )
    jobReport = JobReport(jenkinsInfo, timeStats, stages)
    return jobReport


def addChildJobs(stageTemplate, templates, stage, jobs, server):
    stageFlowUrl = urlparse.urljoin(server, stage["_links"]["self"]["href"])
    log.debug("URL for child jobs for '%s' = %s ", stage["name"], stageFlowUrl)
    stageFlowInfo = getUrl(stageFlowUrl)

    log.info(
        "found %d stage(s) for '%s'",
    ...
    for node in stageFlowInfo["stageFlowNodes"]:
        ...
        logUrl = urlparse.urljoin(server, node["_links"]["log"]["href"])
        ...
        #
        # The following is kind of hacky, but haven't found a better
        # technique given the data returned from Jenkins.
        #
        # What we're trying to do is determine the URL for a downstream job
        # started by the current job.  The info is only available in the
        # 'text' attribute, which unfortunately is free form (and therefore
        ...
        if matchJobNumber and len(matchJobNumber.groups()) == 2:
            ...
            log.warning(
                "Unable to determine number for child job for "
                "step '%s' of job %s", node["name"], jobName
            )
            log.debug("nodeLog['text']=%s", nodeLog["text"])
            continue

        jobUrl = stageTemplate.childInfoUrl\
            .replace("%jobName%", jobName)\
            .replace("%jobNumber%", jobNumber)
        jobUrl = urlparse.urljoin(server, jobUrl)

        jobInfo = getJobInfo(jobUrl)
        childJobReport = buildReport(templates, jobInfo, jobName, server)
...
def addChildJobTemplates(templates, childTemplateName, parentJob):
    ...
    if not found:
        log.warning(
            "No template found for child job name %s", childTemplateName
        )
        return
    ...
    for instance in template.instanceTemplates:
        ...
        jenkinsInfo = JenkinsInfo(
            template.name, instance.jobPrefix, "", "", ""
        )
...
def buildJSONReport(report, data_file):
    ...
    with open(data_file, 'w') as outFile:
        json.dump(
            report.toDict(), outFile,
            default=dumpit, indent=4, sort_keys=True, separators=(',', ': ')
        )
...
def buildJobHTML(job, level):
    ...
    if job.jenkinsInfo and job.jenkinsInfo.url:
        jobLink = "<a href='%s'>%s - %s</a>" % (
            job.jenkinsInfo.url, job.jenkinsInfo.name, job.jenkinsInfo.label
        )
    else:
        jobLink = job.jenkinsInfo.label
    duration = print_duration(job.timeStats.duration) \
        if job.timeStats and job.timeStats.duration else ""
    status = job.jenkinsInfo.status \
        if job.jenkinsInfo and validStatus(job.jenkinsInfo.status) else ""
...
def buildStageHTML(stage, level):
    stageRows = []
    indentLevel = "indent%d" % level
    duration = print_duration(stage.timeStats.duration) \
        if stage.timeStats and stage.timeStats.duration else ""
...
def main(options):
    build = BuildInfo(options.server, options.job_name, options.build)
    templates = loadReportTemplates(options.template, build)
    beginJobInfo = getJobInfo(build.buildUrl)
    if options.job_status:
        beginJobInfo["status"] = options.job_status
    report = buildReport(templates, beginJobInfo, "begin", build.server)
...
if __name__ == '__main__':
    setStdOutLogging(log.DEBUG)
    parser = argparse.ArgumentParser(description='Report build status')
    parser.add_argument(
        '--server', type=str, required=True,
        help='The Jenkins server to the build (http://server)'
    )
    parser.add_argument(
        '--job-name', type=str, required=True,
        help='The full name of the Jenkins job'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '-j',  '--json-output-file', type=str, default='buildReport.json',
        help='Name of the JSON output file; default is buildReport.json'
    )
    parser.add_argument(
        '-html',  '--html-output-file', type=str, default='buildReport.html',
        help='Name of the HTML output file; default is buildReport.html'
    )
    parser.add_argument(
        '-t',  '--template', type=file, default='jobTemplate.json',
