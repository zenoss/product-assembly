...
def configure_logging(name, version, tmpdir):
    ...
    handler = RotatingFileHandler(log_file_name, maxBytes=8192*1024, backupCount=5)
...
def main():
    ...
    parser = parse_options(scriptVersion, scriptName + scriptSummary + documentationURL)
    # Add in any specific parser arguments for %scriptName
    cli_options = vars(parser.parse_args())
    log, logFileName = configure_logging(scriptName, scriptVersion, cli_options['tmpdir'])
