...
def main(args):
    ...
    for zpName in manifest["install_order"]:
        ...
        if not zpFileName:
            ...
            if args.link:
                print "Installing zenpack in link mode: %s %s" % (
                    zpName,
                    zpFile,
                )
                cmd.append("--link")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Install zenpacks from the manifest "
    )
    parser.add_argument(
        "zp_manifest",
        type=file,
    ...
    parser.add_argument(
        "--link", action="store_true", help="link-install the zenpacks"
    )
    parser.add_argument(
        "zp_blacklist",
        type=str,
        nargs="?",
        help="json file with list of zenpacks to exclude from the install",
    )
